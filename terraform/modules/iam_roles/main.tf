...
# -----------------------------------------------------------------------------
# Autoscaler Lambda Role
# -----------------------------------------------------------------------------
resource "aws_iam_role" "lambda_autoscaler_role" {
  name               = "${var.app_name}-${var.environment}-lambda-autoscaler-role"
...
resource "aws_iam_policy" "lambda_autoscaler_policy" {
  name = "${var.app_name}-${var.environment}-lambda-autoscaler-policy"
  policy = jsonencode({
    Version   = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_role_policy_attachment" "lambda_autoscaler_policy_attachment" {
...
resource "aws_iam_policy" "scheduler_policy" {
  ...
  description = "Allows EventBridge Scheduler to invoke the application Lambdas."
    ...
    Statement = [
      {
        ...
        "arn:aws:lambda:${var.aws_region}:${var.aws_account_id}:function:${var.app_name}-${var.environment}-ecs-autoscaler"
        ]
      }
    ]
  })
